...
export default function Home() {
  ...
  return (
    <div className="wiki-shell bg-[#0B0D10] text-white">
        ...
        <div className="flex items-center gap-3 px-4 h-14">
          ...
          <div className="flex-1 max-w-xl mx-auto relative">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder:text-zinc-400 outline-none focus:border-cyan-400/50"
            />
            {q && (
          ...
          <a
            ...
            className="hidden lg:inline-flex items-center gap-1.5 text-xs text-zinc-400 hover:text-white transition-colors"
          >
            source ↗
          </a>
        </div>
          ...
          <div className="lg:hidden fixed inset-0 z-50 flex">
            ...
            <aside className="relative w-72 max-w-[80vw] bg-[#0B0D10] border-r border-white/10 overflow-y-auto p-3 anim-slide">
              <div className="flex items-center justify-between mb-3">
                <span className="mono-label text-zinc-400">NAVIGATION</span>
      ...
      {/* Footer */}
      <footer className="border-t border-white/10 bg-[#0B0D10] px-4 py-5 text-xs text-zinc-400">
    ...
    <>
      <a href="#" onClick={(e) => { e.preventDefault(); go('activate'); }} className="hidden lg:flex items-center gap-2 mb-4 px-2">
        ...
        <div>
          ...
          <div className="text-[10px] text-zinc-400 -mt-1 font-mono">AI SECRET SAUCE WIKI</div>
        </div>
      </a>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                {s.label}
                {s.count !== undefined && (
                  <span className="ml-1.5 text-[10px] font-mono text-zinc-400">{s.count}</span>
                )}
              </div>
              <div className="text-[10px] text-zinc-400 line-clamp-2 leading-tight mt-0.5">{s.desc}</div>
            </div>
          </button>
        ))}
      </nav>
      ...
      <div className="mt-4 pt-3 border-t border-white/10">
        <div className="mono-label text-zinc-400 mb-1.5 px-2">SOURCE</div>
        <a
          href="https://github.com/marktantongco/promptc"
          target="_blank" rel="noopener"
          className="block text-xs text-zinc-400 hover:text-zinc-300 px-2 py-1"
        >
    ...
    <div className="anim-zone">
      <div className="mb-4">
        ...
        <h1 className="wiki-display text-3xl mb-1">
          ...
          <span className="text-base text-zinc-400 ml-2 font-sans">for "{query}"</span>
        </h1>
        <p className="text-xs text-zinc-400">Click any result to jump to its section.</p>
      </div>
        ...
        <div className="space-y-2">
          {results.map((r, i) => (
            ...
            >
              ...
              {r.desc && <p className="text-xs text-zinc-400 line-clamp-1 mb-1">{r.desc}</p>}
